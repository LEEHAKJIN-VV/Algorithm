...
class GraphNode {
    ...
    public GraphNode(String city, double longitude, double latitude,double distance) {
        this.city = city; this.longitude = longitude; this.distance = distance;
        this.latitude = latitude; this.link = null;
    }
...
public class Graph {
        ...
        while (true) {
            ...
            switch (str) {
                ...
                case "Dijkstra":
                    ...
                    System.out.print("목적지 도시 입력: "); ct2 = sc.nextLine();   // 두지점 입력
    ...
    private static void insertVertex(String str) {    //정점 추가
        String[] st = str.split("\t");
        double b1 = Double.parseDouble(st[1]); double b2 = Double.parseDouble(st[2]);
    ...
    private static void dijkstra(String ct1,String ct2){
        ...
        GraphNode src = graphSearch(ct1); GraphNode dest = graphSearch(ct2);  //출발지,목적지
        src.dis = 0;    // 출발 노드 dis[s] = 0 만듬

        while (true){
            GraphNode x = findMin();    // 집합s 에 속하지않으면서 dis 가 최솟값인 노드 반환
            if(x == null) break; // x가  null 이면 집합s 에 다 노드가 포함됐으므로 종료
        ...
        //while 문 조건 종료시 목적지 노드의 div 구해짐
    ...
    private static void relax(GraphNode u, GraphNode v, double distance){
    ...
    private static GraphNode findMin(){  //최솟값 찾는 메소드
        double length = 10000000; String str = null;
        for (int i = 0; i <size ; i++) {
            if(list[i].check) continue;

            if(list[i].dis < length ) {
                length = list[i].dis;
                str = list[i].city;
            }
        }
        if(str == null) return null;
        return (graphSearch(str));
    }
