...
public class Graph {
    private static ArrayList<GraphNode> list2;
    ...
    private static void dijkstra(String ct1, String ct2) {
        for (int i = 0; i < list2.size(); i++) {
            list2.get(i).dis = 1000000;
            list2.get(i).pre = null;
        }
    ...
    private static GraphNode findMin() {  //최솟값 찾는 메소드
        ...
        String str = null;
        for (int i = 0; i < list2.size(); i++) {
            if (list2.get(i).check) continue;

            if (list2.get(i).dis < length) {
                length = list2.get(i).dis;
                str = list2.get(i).city;
            }
        }
    ...
    private static GraphNode graphSearch(String name) { // 지점 이름으로 인접리스트 배열 요소 반환
        for (int i = 0; i < list2.size(); i++) {
            if (list2.get(i).city.equals(name))
                return list2.get(i);
        }
        return null;
    }
